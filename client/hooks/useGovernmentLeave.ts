...
import { GovernmentLeavePolicy, MYANMAR_GOVERNMENT_LEAVE_TYPES } from '@/types/government';
...
export const useGovernmentLeave = () => {
  ...
  const createLeavePolicy = async (policy: Omit<GovernmentLeavePolicy, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      // Check if leave type already exists
      const existing = leavePolicies.find(p => 
        p.leaveType.toLowerCase() === policy.leaveType.toLowerCase() && p.isActive
      );
      
      if (existing) {
        throw new Error(`Leave policy for "${policy.leaveType}" already exists`);
      }
  ...
  const updateLeavePolicy = async (id: string, updates: Partial<GovernmentLeavePolicy>) => {
    try {
  ...
  const getLeavePolicy = (leaveType: string): GovernmentLeavePolicy | undefined => {
    return leavePolicies.find(policy => 
      policy.leaveType.toLowerCase() === leaveType.toLowerCase() && policy.isActive
    );
  };

  const getApplicableLeavePolicies = (grade: number, gender: 'male' | 'female'): GovernmentLeavePolicy[] => {
    return leavePolicies.filter(policy => 
      policy.isActive &&
      policy.applicableGrades.includes(grade) &&
      (policy.applicableGenders.includes('both') || policy.applicableGenders.includes(gender))
    );
  };
  ...
  const validateLeaveRequest = (
    ...
    if (!policy.applicableGrades.includes(grade)) {
      errors.push(`Leave type "${leaveType}" is not applicable for Grade ${grade}`);
    }

    // Check gender eligibility
    if (!policy.applicableGenders.includes('both') && !policy.applicableGenders.includes(gender)) {
      errors.push(`Leave type "${leaveType}" is not applicable for ${gender} employees`);
    }

    // Check minimum service requirement
    if (serviceMonths < policy.minimumServiceRequired) {
      errors.push(`Minimum service requirement of ${policy.minimumServiceRequired} months not met`);
    }

    // Check maximum consecutive days
    if (duration > policy.maxConsecutiveDays) {
      errors.push(`Maximum consecutive days for "${leaveType}" is ${policy.maxConsecutiveDays} days`);
    }
    ...
    if (duration > policy.entitlementPerYear) {
      errors.push(`Requested duration exceeds annual entitlement of ${policy.entitlementPerYear} days`);
    }
  ...
  const generateDefaultLeavePolicies = async () => {
    try {
      ...
      const defaultPolicies: Omit<GovernmentLeavePolicy, 'id' | 'createdAt' | 'updatedAt'>[] = [
        // Annual Leave
        {
          ...
          eligibilityRules: 'Available to all employees after 12 months of service',
          eligibilityRulesMyanmar: 'ဝန်ထမ်းအားလုံး ၁၂လ ဝန်ထမ်းပြုပြီးနောက် ရရှိနိုင်သည်',
          isActive: true,
        ...
        // Maternity Leave
        {
          ...
          eligibilityRules: 'Available to female employees with 6+ months service',
          eligibilityRulesMyanmar: 'ဝန်ထမ်းပြု ၆လ ကျော်ရှိသော အမျိုးသမီးများ ရရှိနိုင်သည်',
          isActive: true,
          effectiveDate: new Date(),
          createdBy: 'system',
          approvedBy: 'admin'
        ...
        // Casual Leave
        {
          ...
          descriptionMyanmar: 'အရေးပေါ် ကိုယ်ရေးကိုယ်တာ ကိစ္စများအတွက် ခဏတာလပ်ရက်',
          eligibilityRules: 'Available for urgent personal matters with minimum notice',
          eligibilityRulesMyanmar: 'အရေးပေါ် ကိုယ်ရေးကိစ္စများအတွက် အနည်းဆုံး ကြိုတင်အကြောင်းကြားမှုဖြင့် ရရှိနိုင်သည်',
          isActive: true,
          effectiveDate: new Date(),
          createdBy: 'system',
          approvedBy: 'admin'
        ...
        // Study Leave
        {
          ...
          eligibilityRules: 'Available to lower grade employees with 2+ years service',
          eligibilityRulesMyanmar: 'အဆင့်နိမ့် ဝန်ထမ်းများ (၂နှစ်ကျော် ဝန်ထမ်းပြုရှိသူများ) ရရှိနိုင်သည်',
          isActive: true,
          effectiveDate: new Date(),
          createdBy: 'system',
          approvedBy: 'admin'
        ...
        // Religious/Pilgrimage Leave
        {
          ...
          descriptionMyanmar: 'ဘာသာရေး ဘုရားဖူးခရီးနှင့် ဝိညာဏရေးရာ ရည်ရွယ်ချက်အတွက် လပ်ရက်',
          eligibilityRules: 'Available once per year with advance notice',
          eligibilityRulesMyanmar: 'တစ်နှစ်လျှင် တစ်ကြိမ် ကြိုတင်အကြောင်းကြားမှုဖြင့် ရရှိနိုင်သည်',
          isActive: true,
          effectiveDate: new Date(),
          createdBy: 'system',
          approvedBy: 'admin'
  ...
  const exportLeavePolicies = (): string => {
    const csvData = leavePolicies.map(policy => ({
      ...
      'Requires Medical Certificate': policy.requiresMedicalCertificate ? 'Yes' : 'No',
