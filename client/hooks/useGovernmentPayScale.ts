...
export const useGovernmentPayScale = () => {
  ...
  const createPayScale = async (
    payScale: Omit<GovernmentPayScale, "id" | "createdAt" | "updatedAt">,
  ) => {
    try {
      // Check if grade/step combination already exists
      const existing = payScales.find(
        (scale) =>
      ...
      if (existing) {
        throw new Error(
          `Pay scale for Grade ${payScale.grade}, Step ${payScale.step} already exists`,
        );
      }
  ...
  const getPayScaleByGradeStep = (
    ...
    return payScales.find(
      (scale) => scale.grade === grade && scale.step === step && scale.isActive,
    );
  };
  ...
  const validatePayScale = (
    payScale: Partial<GovernmentPayScale>,
  ): string[] => {
    const errors: string[] = [];
  ...
  const importPayScales = async (
    payScales: Omit<GovernmentPayScale, "id" | "createdAt" | "updatedAt">[],
  ) => {
    try {
      setLoading(true);
      const batch = [];
      ...
      for (const payScale of payScales) {
        ...
        if (validationErrors.length > 0) {
          throw new Error(
            `Validation failed for Grade ${payScale.grade}, Step ${payScale.step}: ${validationErrors.join(", ")}`,
          );
        }
          ...
          defaultScales.push({
            ...
            allowances: {
              ...
              responsibilityAllowance:
                grade >= 15 ? Math.floor(baseSalary * 0.15) : 0,
              riskAllowance: 0,
              specialAllowance: 0,
            },
            benefits: {
...
export const usePersonnelGrade = (personnelId?: string) => {
  const [personnelGrade, setPersonnelGrade] = useState<PersonnelGrade | null>(
    null,
  );
  ...
  const createPersonnelGrade = async (
    grade: Omit<PersonnelGrade, "id" | "createdAt" | "updatedAt">,
  ) => {
    try {
