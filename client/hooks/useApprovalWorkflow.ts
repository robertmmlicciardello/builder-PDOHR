...
export const useApprovalWorkflow = (workflowId?: string) => {
  ...
  const fetchWorkflow = async (id: string) => {
    try {
      setLoading(true);
      const workflowDoc = await getDocs(
        query(collection(db, "approvalWorkflows"), where("__name__", "==", id)),
      );
      ...
      if (!workflowDoc.empty) {
        ...
        setWorkflow({
          ...
          workflowHistory:
            data.workflowHistory?.map((h: any) => ({
              ...h,
              performedAt: h.performedAt?.toDate(),
            })) || [],
          approvalLevels:
            data.approvalLevels?.map((l: any) => ({
              ...l,
  ...
  const fetchUserWorkflows = async (userId: string, status?: string) => {
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        const userInvolved = data.approvalLevels?.some(
          (level: any) =>
            level.approverId === userId ||
            level.approverRole === "department_head", // Add role-based filtering
        );
  ...
  const approveRequest = async (
    workflowId: string,
    comments: string,
    approverId: string = "current-user",
  ) => {
    try {
      if (!workflow) return;
  ...
  const rejectRequest = async (
    workflowId: string,
    reason: string,
    rejectorId: string = "current-user",
  ) => {
    try {
      if (!workflow) return;
  ...
  const isCurrentUserApprover = (userId: string): boolean => {
    ...
    return (
      ...
      currentApprover?.approverRole === "department_head"
