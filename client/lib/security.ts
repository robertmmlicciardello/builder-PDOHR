...
export const SECURITY_CONFIG = {
  ...
  ENCRYPTION_KEY:
    ...
    "pdf-hr-system-2024-secure-key-32-chars",
...
export const VALIDATION_PATTERNS = {
  ...
  XSS_PATTERNS:
    /<script|javascript:|onload=|onerror=|onclick=|onmouseover=|<iframe|<object|<embed|<link|<meta/i,
} as const;
...
export class SecurityUtils {
  ...
  static validateInput(
    input: string,
    type: "email" | "password" | "text" | "phone" | "personnel_id" = "text",
  ): {
    isValid: boolean;
    sanitized: string;
    errors: string[];
  ...
  static generateCSRFToken(): string {
    return CryptoJS.lib.WordArray.random(
      SECURITY_CONFIG.CSRF_TOKEN_LENGTH,
    ).toString();
  }
  ...
  static validateFileUpload(file: File): {
    ...
    if (
      /\.(exe|bat|cmd|scr|pif|com|jar|vbs|js|html|htm|php|asp|jsp)$/i.test(
        file.name,
      )
    ) {
